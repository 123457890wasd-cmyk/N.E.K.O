...
def test_user_initiated_invite_opens_game_with_no_pending():
    """No pending invite + user-initiated intent → open_game (the bug fix path).

    Pre-fix this was a no-op: the user's "我们玩" / "let's play" message fell
    through to the LLM and the model would role-play a game in text. With the
    new Path 2 the hook synthesizes a pending state, immediately applies
    accept, and returns action=open_game with a real game_url — so the
    frontend launches a game window directly without showing the 来一局 prompt.
    """
    for text in (
        '来玩',
        '我们玩',
        ...
        result = sr._maybe_apply_mini_game_invite_keyword(LANLAN, text)
        assert result is not None, f"{text!r} should trigger user-initiated invite"
        assert result['action'] == 'open_game', f"{text!r} → {result}"
        assert result.get('game_url'), f"{text!r} missing game_url"
        assert result['game_type'] in sr.MINI_GAME_INVITE_AVAILABLE_GAMES


def test_user_initiated_invite_picks_named_game():
    """A user-named game (e.g. 踢足球) wins over the random fallback."""
    for text, expected in (
        ('来玩一局羽毛球', 'badminton'),
        ('踢足球吧', 'soccer'),
        ('play badminton please', 'badminton'),
        ("let's play football", 'soccer'),
        ('打羽毛球', 'badminton'),
        ('play soccer', 'soccer'),
    ):
        result = sr._maybe_apply_mini_game_invite_keyword(LANLAN, text)
        assert result is not None, f"{text!r} should match invite"
        assert result['action'] == 'open_game', f"{text!r} → {result}"
        assert result['game_type'] == expected, (
            f"{text!r} → game_type={result['game_type']}, expected {expected}"
        )
        assert result['game_url'].startswith(f'/{expected}_demo?'), (
            f"{text!r} → game_url={result['game_url']}"
        )


def test_user_initiated_invite_with_decline_keywords_returns_none():
    """decline keywords in the same text override invite (decline > invite)."""
    for text in (
        '算了不玩',
        '算了不玩足球',
        '不想玩',
        ...
        result = sr._maybe_apply_mini_game_invite_keyword(LANLAN, text)
        assert result is None, f"{text!r} should be refused, got {result}"


def test_user_initiated_invite_with_later_keywords_returns_none():
    """later keywords override invite (later > invite)."""
    for text in (
        '等会儿再玩',
        'later play a game',
        '나중에 놀자',
        ...
        result = sr._maybe_apply_mini_game_invite_keyword(LANLAN, text)
        assert result is None, f"{text!r} should be deferred, got {result}"


def test_user_initiated_invite_unrelated_text_returns_none():
    """Text without invite/decline/later keywords stays a no-op."""
    for text in (
        '今天天气不错',
        '你好',
        'hello',
        ...
        result = sr._maybe_apply_mini_game_invite_keyword(LANLAN, text)
        assert result is None, f"{text!r} should not trigger, got {result}"


def test_user_initiated_invite_disabled_globally_returns_none(monkeypatch):
    """MINI_GAME_INVITE_ENABLED=False gates Path 2 (matches Path 1 / delivery)."""
    monkeypatch.setattr(sr, 'MINI_GAME_INVITE_ENABLED', False)
    result = sr._maybe_apply_mini_game_invite_keyword(LANLAN, '我们玩')
    assert result is None


def test_user_initiated_invite_with_pending_takes_response_path():
    """Pending invite + user text → original accept/decline/later response path.

    The user-initiated Path 2 must NOT fire when a pending invite exists —
    otherwise Path 1 (response keywords) would be shadowed and the
    pending_session_id would be regenerated. Verify Path 1 still wins.
    """
    state = sr._mini_game_invite_get_state(LANLAN)
    state['delivered_at'] = time.time() - 1
    state['responded_at'] = None
    state['pending_session_id'] = 'pending-sess-xyz'
    state['last_game_type'] = 'soccer'

    result = sr._maybe_apply_mini_game_invite_keyword(LANLAN, '好啊')
    assert result['action'] == 'open_game'
    # session_id comes from the pending invite, NOT regenerated by Path 2.
    assert result['session_id'] == 'pending-sess-xyz'


def test_user_initiated_invite_marks_ever_delivered_in_memory():
    """User-initiated invite marks _invite_ever_delivered so force-first doesn't re-fire."""
    from main_logic.proactive_chat import state as sr_history

    assert LANLAN not in sr_history._invite_ever_delivered
    sr._maybe_apply_mini_game_invite_keyword(LANLAN, '我们玩')
    assert sr_history._invite_ever_delivered.get(LANLAN) is True


def test_keywords_have_invite_category_for_all_native_locales():
    """Every native locale must define a non-empty invite list.

    Mirrors the existing accept/decline/later coverage test; user-initiated
    invites are now part of the keyword contract.
    """
    ...
    for lang in ('zh', 'zh-TW', 'en', 'ja', 'ko', 'ru', 'es', 'pt'):
        ...
        invite = MINI_GAME_INVITE_KEYWORDS[lang].get('invite', [])
        assert isinstance(invite, list) and invite, (
            f"{lang}/invite 关键词列表空"
        )
